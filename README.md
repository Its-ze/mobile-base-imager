...
- Fedora-compatible x86_64 `.AppImage` package;
...
```bash
chmod +x Mobile_Base_Imager-0.4.0-x86_64.AppImage
./Mobile_Base_Imager-0.4.0-x86_64.AppImage
```
...
```bash
sudo apt install ./mobile-base-imager_0.4.0_linux_amd64.deb
```
...
New Mobile Base 0.9.5 units automatically use DHCP when Ethernet is connected. Open the unique `.local` setup address printed in `mobile-base-pairing.txt`, enter the pairing code, choose Ethernet-only setup, and configure `mobileadmin`. The dashboard is on port 8788 and the full pinned SSH shell is on port 22. USB NMEA GPS receivers and supported ESP32 modules are detected through stable paths; known modules are backed up before verified firmware flashing, then supervised as independent workers. An automatic field mission starts or resumes, and the Tab5 can pair as a network controller without embedding a shared public key. A card may also be privately provisioned before first boot with a single-use hub enrollment phrase and an operator SSH public key; first boot installs the key, consumes enrollment files after setup, and disables password authentication. The public image contains no enrollment secrets, controller keys, or operator keys. Remote management is outbound-only and opt-in; the image does not expose a hidden Internet listener.
