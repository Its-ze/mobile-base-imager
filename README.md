...
```bash
chmod +x Mobile_Base_Imager-0.3.6-x86_64.AppImage
./Mobile_Base_Imager-0.3.6-x86_64.AppImage
```
...
```bash
sudo apt install ./mobile-base-imager_0.3.6_linux_amd64.deb
```
...
New Mobile Base 0.9.1 units automatically use DHCP when Ethernet is connected. Open the unique `.local` setup address printed in `mobile-base-pairing.txt`, enter the pairing code, choose Ethernet-only setup, and set the `mobileadmin` password. The dashboard is on port 8788 and the full pinned SSH shell is on port 22. Units can also enroll from their local **Hub** page with the authenticated Unraid home base at [mobilebase.itsz.studio](https://mobilebase.itsz.studio). Remote management is outbound-only and opt-in; the image does not expose a hidden Internet listener.
